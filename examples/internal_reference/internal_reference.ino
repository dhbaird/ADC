...
float get_voltage(uint8_t trim) {
    ...
    for(int i=0; i<NUM_AVGS; i++) {
        average += 1.195/adc->analogRead(ADC_INTERNAL_SOURCE::VREF_OUT)*(adc->getMaxValue(ADC_NUM::ADC_0));
    }
...
void setup() {
    ...
    Serial.print(1.195/adc->analogRead(ADC_INTERNAL_SOURCE::VREF_OUT)*adc->getMaxValue(ADC_NUM::ADC_0), 5);
    Serial.println(" V.");


    Serial.print("Bandgap value: ");
    //Serial.print(3.3*adc->analogRead(ADC_INTERNAL_SOURCE::BANDGAP)/adc->getMaxValue(ADC_NUM::ADC_0), 5);
    adc->adc0->analogRead(ADC_INTERNAL_SOURCE::BANDGAP);
    adc->adc0->differentialMode(); // Use differential mode for better precision.
    Serial.print(voltage_target*adc->adc0->readSingle()/adc->getMaxValue(ADC_NUM::ADC_0), 5);
